...
class _DiscountsState extends State<Discounts> {
  ...
  void initState() {
    super.initState();
    isExpandedList =
        List<bool>.generate(discountsData.length, (index) => false);
  }
    ...
    return BlocProvider(
      ...
      child: BlocBuilder<GetExpensesBloc, GetExpensesState>(
        ...
        if (state is GetExpensesLoading) {
          ...
          return const Center(child: Text('Failed to load receipts.'));
        } else if (state is GetExpensesSuccess) {
          final currentReceipts = state.expenses.length;

          final currentReceiptsAdjusted = currentReceipts > targets.last
              ? (currentReceipts - targets.last)
              : currentReceipts;

          int nextTarget = targets.firstWhere(
            (target) => currentReceiptsAdjusted < target,
            orElse: () => targets.last,
          );
          int receiptsToNextTarget = nextTarget - currentReceiptsAdjusted;

          int previousTarget = targets.indexOf(nextTarget) > 0
              ? targets[targets.indexOf(nextTarget) - 1]
              : 0;
          double progress = (currentReceiptsAdjusted - previousTarget) /
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                currentReceipts == targets.last
                                    ? "Congratulations! You have reached the maximum target of ${targets.last} receipts!"
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, int i) {
                            bool isUnlocked = currentReceipts >=
                                discountsData[i]['requiredReceipts'];
                            return Padding(
